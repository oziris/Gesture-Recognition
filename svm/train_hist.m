function [svm_model, data_trans] = train_hist(train_data, train_label, normalization)
%TRAIN_HIST Train svm model with a histogram intersection kernel.
%
% [svm_model, data_trans] = train_hist(train_data, train_label, normalization)
% 
...
for i=1:length(C_vals)
    params = sprintf('-t 4 -v 5 -c %g', C_vals(i) );
...
% train model
params = sprintf('-t 4 -c %g', bestc );

tic;
svm_model = libsvmtrain(train_label, train_data, params) ;
toc

end %function train_hist